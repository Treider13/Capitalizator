#!/usr/bin/env bash
# Fusion SQLite + referenced immutable archives every 6h. cron (user trader):
...
docker compose run --rm --no-deps -T -v "$DATA/backups:/backups" fusion \
  python -m capitalizator.fusion.backup --userdir /data/fusion --dest "/backups/fusion-$STAMP" \
  || { echo "backup pack failed at $STAMP" >&2; exit 1; }
find "$DATA/backups" -maxdepth 1 -name 'fusion-*' -mtime +14 -exec rm -rf {} +
echo "backup ok: $DATA/backups/fusion-$STAMP"
