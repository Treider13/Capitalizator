...
Актуальный [аудит по связанным skills](docs/FUSION-SKILLS-AUDIT-2026-09-06.md)
...
Повторный аудит: причины аварийной остановки учитываются независимо и снимаются
только после проверки конкретной версии инцидента. Разрывы истории не создают
FVG/BAG: структура использует непрерывный участок, пока REST не восстановит данные.
На графике в контексте видны history_gaps и длина непрерывного участка.
Отсутствующие Greeks при ненулевом опционном OI означают недоступный контекст.
Политика `fusion-4-evidence-integrity` начинает новую эпоху обучения/квалификации.
