...
| Обязательный sweep | Выход за предшествующий диапазон с возвратом до создания контракта |
...
| Gamma | Достоверная подписанная gamma-экспозиция не подключена и не выдумывается; поле явно показывает отсутствие данных об опционных позициях |
| Новости | Объявления Bybit, прозрачный словарный NLP-фильтр и макрокалендарь Capitalizator; время первого наблюдения сохраняется |
...
необходимо поддерживать актуальным; время загрузки заголовка не заменяет время CPI/FOMC.
...
Неполная восстановленная история позиции блокирует такой допуск. Эти проверки
...
Стоп располагается за экстремумом sweep с отступом на основе хвоста прогнозного
распределения и спреда. Это не делает его «невидимым» и не гарантирует цену исполнения.
...
Рыночные потоки, исполнитель, обучение, архив, новости и обслуживание работают
раздельно. NumPy выполняет математику в нативном коде; наличие Python-потоков не
обещает линейного ускорения CPU из-за GIL. В Docker число потоков BLAS ограничено.
...
Локальная проверка этой реализации 6 сентября 2026: **2208 тестов прошли, 2
пропущены** (пропуски уже присутствовали в исходном наборе). Добавлены 53 проверки
fusion. Ruff проходит по `src` и `tests`; проходят исходная настроенная область
mypy и отдельная строгая проверка 19 модулей fusion. Проверены синтаксис JavaScript,
состав Compose и сборка wheel с включённым HTML консоли. Docker отсутствует в
среде проверки, поэтому запуск контейнера не подтверждён.
