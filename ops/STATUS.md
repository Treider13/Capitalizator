...
Дата проверки: 2026-08-31. Локально: **633 passed, 1 skipped** (skip = нет egress на `api.bybit.com`). GitHub Actions на ветке — **startup_failure**. Это не «CI зелёный».
...
| 0.1.5 parquet | `tests/recorder/test_parquet_sink.py` | не нужно | `mkstemp`+fd+inode `replace`; lock `O_NOFOLLOW` + `O_NONBLOCK` + `S_ISREG` (FIFO отказ); mkdir по одному имени (не в dir-симлинк); два писателя — flock + перечит. Счётчик — `metadata.num_rows` через fd. Зелёный после прогона |
...
| знания / бэкап | `tests/ops/test_vault_backup.py` | VPS нет | Snapshot: serialize + mkstemp. DB: `O_EXCL|O_NOFOLLOW`, inode после `connect` и после `replace`. LAYOUT через `write_regular_text`. `ensure_real_parent` смотрит каждого предка: `mkdir(parents=True)` пишет в цель dir-симлинка, `parent.is_symlink()` после этого врёт. Симлинк / FIFO / hardlink / секрет — отказ. Restore — staging + rename. Живого диска VPS **нет** |
