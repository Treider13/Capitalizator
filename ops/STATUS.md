...
Дата проверки: 2026-08-31. Локально: **637 passed, 1 skipped** (skip = нет egress на `api.bybit.com`). GitHub Actions на ветке — **startup_failure**. Это не «CI зелёный».
...
| знания / бэкап | `tests/ops/test_vault_backup.py` | VPS нет | Snapshot: serialize + mkstemp. DB: `O_EXCL|O_NOFOLLOW`, connect через `/proc/self/fd` (не `connect(path)` — тот ходит в симлинк). Staging — `mkdtemp`. `ensure_real_parent` смотрит каждого предка. Симлинк / FIFO / hardlink / секрет — отказ. Живого диска VPS **нет** |
