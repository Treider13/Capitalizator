...
Дата проверки: 2026-08-31. Локально: **635 passed, 1 skipped** (skip = нет egress на `api.bybit.com`). GitHub Actions на ветке — **startup_failure**. Это не «CI зелёный».
...
| знания / бэкап | `tests/ops/test_vault_backup.py` | VPS нет | Snapshot: serialize + mkstemp. DB: `O_EXCL|O_NOFOLLOW`, inode после `connect` и после `replace`. LAYOUT через `write_regular_text`. `ensure_real_parent` смотрит каждого предка. Staging — `mkdtemp`, не `.{имя}.{pid}`; очистка не делает `rmtree` через симлинк. Симлинк / FIFO / hardlink / секрет — отказ. Restore — staging + rename. Живого диска VPS **нет** |
