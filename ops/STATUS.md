...
| CAV запись | `tests/patterns/test_cav.py`, `test_bar_quality.py`, `test_width.py`, `test_exam.py` | не нужно | REJECT/THROUGH/COMPRESS/DRIFT/NOISE на закрытой свече. stagnant/illiquid → NOISE; разрыв режет ATR. `atr()` — последнее окно по `(close_ts, open_ts)`, не по порядку списка; смесь symbol/tf — отказ. last-2/last-5 — тот же symbol/tf; чужой ноль или close в окне не сосед; чужой vol=1 между двумя нулями не ломает ILLIQUID. Поздний ETH/1h на 80 не обнуляет сегмент; поздний на open current не прячет настоящий jump (в т.ч. 1h). REJECT/THROUGH/COMPRESS/DRIFT last-2 не переворачиваются чужим нулём или vol=1 (в т.ч. ETH/1h current и resistance). Quality/HTF не только REJECT/THROUGH/COMPRESS: мёртвый DRIFT и resistance HTF-against DRIFT → NOISE. Бар с `current.open < close_ts < current.close` — prior для last-2/last-5 и last-prior jump (не только ATR/width). last-2/last-5 — `(close_ts, open_ts)`, не порядок списка и не `open_ts` (в т.ч. last-5). HTF `unknown` и with-us не только THROUGH/COMPRESS (в т.ч. resistance). `w_rank` n — наблюдения, не уникальные `ts`. Exam best-5 — ровно 5 дней (6 равных → 5/6; ничья за 5-е не тянет 6-й). COMPRESS на ребре зоны (close = lo/hi). Vol-rank IC — midrank на pred и на actual, не порядковый номер ничьей. Exam `n` — все кейсы, не размер residual. Плоский бар → `w_now=0`, не `None`; плоский после jump → `None`, не `0`. Residual — |err|/ATR, не знак. Direction hit — знак vs last, не vs pred. `w_now`/`w_rank`/`session_hour` — журнал, не вход. UTC час — `require_utc`, не хардкод `+3` (`+9 16:30` → 7; `+5:30` now/sample в `w_rank`; смещённый prior 07:30Z закрывает ATR/stagnant) |
...
| консоль ноут | `tests/ops/test_console.py` | туннеля нет | GET `/` и `/api/status`; POST `/contour` и `/api/contour` только `action=on`; POST `/order` и PUT/DELETE/PATCH 405; только 127.0.0.1. Без LAYOUT не встаёт. Поля `vps` нет. Симлинк `desk.sqlite` / `tape/` — отказ, 500 `error`. Пустой `desk.sqlite` не дописывает схему. Signer не импортирован. Кнопка «Включить контур» без суток ленты — 409, `trading_mode` остаётся off |
| контур запись | `tests/ops/test_contour.py` | живых суток нет | `hours24` = `check_uptime --hours 24 --max-unmarked-gap-s 0`. Без суток `enable` отказ, `phase.yaml` не пишется. После суток meta `contour=on`. `observe` при off ничего не пишет. При on: лента+CAV+ZLG+BTC+жюри; n<20 → SILENCE. `propose`/signer не импортирует. Ордера нет |
