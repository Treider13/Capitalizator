...
Дата проверки: 2026-09-01. Локально: **944 passed, 2 skipped** (skip = нет egress на `api.bybit.com`; нет бинаря gitleaks). GitHub Actions на ветке — **startup_failure**. Это не «CI зелёный».
...
| CAV запись | `tests/patterns/test_cav.py`, `test_bar_quality.py`, `test_width.py`, `test_exam.py` | не нужно | REJECT/THROUGH/COMPRESS/DRIFT/NOISE на закрытой свече. stagnant/illiquid → NOISE; разрыв режет ATR. `atr()` — последнее окно по `(close_ts, open_ts)`, не по порядку списка; смесь symbol/tf — отказ. last-2/last-5 — тот же symbol/tf; чужой ноль или close в окне не сосед; чужой vol=1 между двумя нулями не ломает ILLIQUID. Поздний ETH/1h на 80 не обнуляет сегмент; поздний на open current не прячет настоящий jump (в т.ч. 1h). REJECT/THROUGH/COMPRESS/DRIFT last-2 не переворачиваются чужим нулём или vol=1 (в т.ч. ETH/1h current и resistance). Quality/HTF не только REJECT/THROUGH/COMPRESS: мёртвый DRIFT и resistance HTF-against DRIFT → NOISE. `w_now`/`w_rank`/`session_hour` — журнал, не вход. UTC час — `require_utc`, не хардкод `+3` (`+9 16:30` → 7; `+5:30` now/sample в `w_rank`; смещённый prior 07:30Z закрывает ATR/stagnant) |
