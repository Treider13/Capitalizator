...
**База:** `main` @ `1144d91` и выше. Сверка предложений из `docs/INDUSTRIAL-RE-2026.md` с цепочкой вызовов.  
**Пункт 1 закрыт** в `70f598f` (desk OMS: `sl_retry` → `sl_unconfirmed`). Ядро A и signer не менялись.
...
- **Статус:** Реализовано (`70f598f`)
- **Что влито в main:** `DeskLoop._handle_stop_missing` — первый снимок `stop_missing` → OMS `amend_stop` / `sl_retry` с `twin.stop`; следующий reconcile `at` всё ещё без стопа → OMS `flatten` / `sl_unconfirmed` + `paper.flatten`. Состояние `_sl_ack` в meta, переживает рестарт. Signer не тронут. Тесты: `tests/desk/test_exchange_sync.py` (`test_missing_venue_stop_*`). Прогон: 1880 passed, 4 skipped.
- **Ручная проверка (demo):** филл → в `exchange_state` пустой `stopLoss` / `stop_missing` → на одном `at` в `oms_commands` есть `amend_stop`/`sl_retry` → новый `at` без стопа → `flatten`/`sl_unconfirmed`, баннер «ПОЗИЦИЯ ЗАКРЫТА». Рестарт между снимками грацию не сбрасывает.
- **Обоснование (на момент аудита, до `70f598f`):**
...
|---|---|---|---|
| 1. SL-ack → flatten | Реализовано (`70f598f`) | — | 0 |
...
- **Влито:** пункт 1 — desk OMS, не Quant Flow в signer: `amend_stop` (`sl_retry`) на первом снимке, `flatten` (`sl_unconfirmed`) на следующем reconcile. Коммит `70f598f` на `main`.
- **Отложить / не делать:** пункты 2–6 в предложенной форме. Журнал отвергнутых, 2-часовой veto, учёт B и PIT-закон уже были в коде. EWMA-памп и «3 veto → hold» либо дублируют ОКО/календарь, либо вредят частоте A.
- **Внешний LLM не нужен.** Существующий `intel.LLMClient` остаётся экстрактором в отдельном процессе.
- После `70f598f` открытых дыр из этого аудита нет. Пункты 2–6 не требуют кода.
