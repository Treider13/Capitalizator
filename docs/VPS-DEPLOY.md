...
Одна для всех процессов: переменная `CAP_UNIVERSE` в `infra/deploy/.env` (`week0` — BTC+ETH, канон
до суток чистой ленты; `desk` — `infra/universe.yaml` или применённый через «Управление»
`/data/universe.yaml`). Рекордер, стол, сигнер и консоль читают одно и то же; стол не ждёт ленту
по символам, которых рекордер не пишет. Смена — правка `.env` и `bash deploy.sh`.
