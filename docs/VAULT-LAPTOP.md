...
Дыры, которые тест ломает (перепроверка 31.08):

- Секрет в файле / строке журнала; лишний файл в `secrets/`.
- Сломанная хеш-цепочка; `PRAGMA integrity_check` после snapshot.
- Лишний / пропавший файл; подмена sha256; непустой dest.
- **Симлинк в `tape/` или слое** — `copytree` по умолчанию *вклеивает содержимое цели* (можно утащить ключ). Отказ.
...
- **Живой `shutil.copy` sqlite** — порча при открытой записи (документация SQLite / практика Freqtrade: не копировать файл на горячую). Пишем через `Connection.backup`.
- **`verify`/`pack` не создают sqlite в источнике**, если его не было.
- Консоль: нет поля `vps: false` (это была выдумка); без `LAYOUT` сервер **не** рисует хранилище; PUT/DELETE/PATCH = 405.
