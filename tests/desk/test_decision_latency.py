"""Э4: journal decision_ms is jury/close − touch.ts, published as latency_decision."""
...
def test_jury_journals_decision_ms(tmp_path: Path) -> None:
    ...
    expected = ((close_ts + timedelta(microseconds=1)) - live.ts).total_seconds() * 1000
    assert row["decision_ms"] == expected
    report = decision_report(desk.knowledge.journal_rows())
    assert report["n"] == 1
    assert report["p50_ms"] == expected
    raw = desk.knowledge.meta("latency_decision")
    assert raw is not None
    meta = json.loads(raw)
    assert meta["n"] == 1
    assert meta["p50_ms"] == expected
